...
class StripeService implements PaymentServiceInterface
{
    ...
    public function processPayment(Transaction $transaction, array $paymentData = []): array
    {
        if (!$transaction->external_id) {
            throw new RuntimeException('Transaction has no external payment intent ID');
        }
        ...
        try {
            ...
            if ($session->payment_status === 'paid') {
                ...
                if ($order) {
                    ...
                    $stripePaymentMethod = PaymentMethod::where('provider', PaymentProviderEnum::STRIPE->value)
